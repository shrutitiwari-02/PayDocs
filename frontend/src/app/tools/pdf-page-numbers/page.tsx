...
import { Upload, FileText, Loader2, Download, Trash2, ListOrdered } from 'lucide-react';
...
export default function PDFPageNumbersTool() {
  ...
  const handleProcess = async () => {
    ...
    try {
      ...
      const res = await fetch('http://localhost:3001/api/pdf/page-numbers', {
        method: 'POST',
        body: formData
      });
      ...
      toast({ title: 'Success', description: 'Your PDF with page numbers has been downloaded.' });
    } catch (error: any) {
    ...
    <div className="container mx-auto px-4 py-8">
      <div className="text-center mb-8">
        <h1 className="text-4xl font-bold text-slate-900 mb-4 flex items-center justify-center gap-3">
          <ListOrdered className="h-10 w-10 text-indigo-600" />
          Add Page Numbers
        </h1>
        <p className="text-slate-600 text-lg">Automatically stamp page numbers at the bottom of every page.</p>
      </div>

      <Card className="max-w-xl mx-auto shadow-xl border-0 bg-white/60 backdrop-blur-xl">
        <CardHeader>
          <CardTitle>Select PDF</CardTitle>
          <CardDescription>Upload a PDF document to add page numbers.</CardDescription>
        </CardHeader>
          ...
          {file && (
            <div className="space-y-4 pt-4 border-t border-slate-100">
              <Button onClick={handleProcess} disabled={isProcessing} className="w-full bg-indigo-600 hover:bg-indigo-700 h-12 text-lg rounded-xl">
                {isProcessing ? <Loader2 className="mr-2 h-5 w-5 animate-spin" /> : <ListOrdered className="mr-2 h-5 w-5" />}
                Add Page Numbers
              </Button>
            </div>
          )}
