...
import { Upload, FileText, Loader2, Download, Trash2, Droplet } from 'lucide-react';
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      <div className="text-center mb-8">
        ...
        <p className="text-slate-600 text-lg">Stamp a custom text watermark across all pages of your PDF.</p>
      </div>

      <Card className="max-w-xl mx-auto shadow-xl border-0 bg-white/60 backdrop-blur-xl">
        <CardHeader>
          <CardTitle>Select PDF & Text</CardTitle>
          <CardDescription>Upload a PDF document to watermark.</CardDescription>
        </CardHeader>
          ...
          {file && (
            <div className="space-y-4 pt-4 border-t border-slate-100">
              <div className="space-y-2">
                ...
                <Input value={watermarkText} onChange={(e) => setWatermarkText(e.target.value)} placeholder="e.g. CONFIDENTIAL" className="h-12" />
              </div>
              <Button onClick={handleProcess} disabled={isProcessing || !watermarkText} className="w-full bg-indigo-600 hover:bg-indigo-700 h-12 text-lg rounded-xl">
                {isProcessing ? <Loader2 className="mr-2 h-5 w-5 animate-spin" /> : <Droplet className="mr-2 h-5 w-5" />}
                Add Watermark
              </Button>
            </div>
          )}
